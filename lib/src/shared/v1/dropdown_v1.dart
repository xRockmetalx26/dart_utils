...
const double _kMenuItemHeight = 32;
...
///
/// Used by [DropdownButtonV1.selectedItemBuilder].
...
class _DropdownMenuItemButtonState<T>
  ...
  void _handleOnTap() {
    final DropdownMenuItemV1<T> dropdownMenuItem =
...
class _MenuItem<T> extends SingleChildRenderObjectWidget {
  ...
  final DropdownMenuItemV1<T>? item;
...
/// An item in a menu created by a [DropdownButtonV1].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class DropdownMenuItemV1<T> extends _DropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const DropdownMenuItemV1({
    super.key,
    this.onTap,
    this.value,
    this.enabled = true,
  ...
  ///
  /// Eventually returned in a call to [DropdownButtonV1.onChanged].
  final T? value;
...
/// An inherited widget that causes any descendant [DropdownButtonV1]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [DropdownButtonV1] widgets placed within material data tables, as
/// required by the Material Design specification.
class DropdownButtonHideUnderlineV1 extends InheritedWidget {
  /// Creates a [DropdownButtonHideUnderlineV1]. A non-null [child] must
  /// be given.
  const DropdownButtonHideUnderlineV1({
    super.key,
    required super.child,
  });

  /// Returns whether the underline of [DropdownButtonV1] widgets should
  /// be hidden.
  static bool at(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<
            DropdownButtonHideUnderlineV1>() !=
        null;
  }

  @override
  bool updateShouldNotify(DropdownButtonHideUnderlineV1 oldWidget) => false;
}
...
/// To update from [DropdownButtonV1] to [DropdownMenu], you will
/// need to make the following changes:
///
/// 1. Instead of using [DropdownButtonV1.items], which
/// takes a list of [DropdownMenuItemV1]s, use
/// [DropdownMenu.dropdownMenuEntries], which
/// takes a list of [DropdownMenuEntry]'s.
///
/// 2. Instead of using [DropdownButtonV1.onChanged],
...
/// the [DropdownButtonV1.value] property, you can set the
...
/// from [DropdownButtonV1] to [DropdownMenu].
///
/// ## Using [DropdownButtonV1]
...
/// Typically, an enum is used. Each [DropdownMenuItemV1] in [items] must be
...
///
/// {@tool dartpad}
/// This sample shows a [DropdownButtonV1] with a large arrow icon,
...
///
/// {@tool dartpad}
/// This sample shows how you would rewrite the above [DropdownButtonV1]
...
/// See also:
///
///  * [DropdownButtonFormFieldV1], which integrates with the [Form] widget.
///  * [DropdownMenuItemV1], the class used to represent the [items].
///  * [DropdownButtonHideUnderlineV1], which prevents its descendant dropdown buttons
///    from displaying their underlines.
///  * [ElevatedButton], [TextButton], ordinary buttons that trigger a single action.
///  * <https://material.io/design/components/menus.html#dropdown-menu>
class DropdownButtonV1<T> extends StatefulWidget {
  /// Creates a dropdown button.
  ///
  /// The [items] must have distinct values. If [value] isn't null then it
  /// must be equal to one of the [DropdownMenuItemV1] values. If [items] or
  ...
  DropdownButtonV1({
    ...
    // DropdownButtonFormField.
          ...
          items == null ||
              items.isEmpty ||
              value == null ||
              items.where((DropdownMenuItemV1<T> item) {
  ...
  final List<DropdownMenuItemV1<T>>? items;

  /// The value of the currently selected [DropdownMenuItemV1].
  ///
  /// If [value] is null and the button is enabled, [hint] will be displayed
  /// if it is non-null.
  ///
  ...
  ///
  /// If the [onChanged] callback is null or the list of [DropdownButtonV1.items]
  /// is null then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [DropdownButtonV1.disabledHint] widget if it is non-null.
  /// If [DropdownButtonV1.disabledHint] is also null but [DropdownButtonV1.hint] is
  /// non-null, [DropdownButtonV1.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  /// [DropdownMenuItemV1]s in [items].
  ///
  /// When a [DropdownMenuItemV1] is selected, the widget that will be displayed
  /// from the list corresponds to the [DropdownMenuItemV1] of the same index
  /// in [items].
  ///
  /// {@tool dartpad}
  /// This sample shows a `DropdownButton` with a button with [Text] that
  /// corresponds to but is unique from [DropdownMenuItemV1].
  ///
  /// ** See code in examples/api/lib/material/dropdown/dropdown_button.selected_item_builder.0.dart **
  /// {@end-tool}
  ///
  /// If this callback is null, the [DropdownMenuItemV1] from [items]
  ...
  ///
  /// As the padding increases, the size of the [DropdownButtonV1] will also
  ...
  @override
  State<DropdownButtonV1<T>> createState() => _DropdownButtonV1State<T>();
}

class _DropdownButtonV1State<T> extends State<DropdownButtonV1<T>>
  ...
  @override
  void didUpdateWidget(DropdownButtonV1<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (widget.focusNode == null) {
      _internalNode ??= _createFocusNode();
    }
  ...
  void _updateSelectedIndex() {
    ...
    assert(
      widget.items!
              .where((DropdownMenuItemV1<T> item) => item.value == widget.value)
              .length ==
          1,
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (!DropdownButtonHideUnderlineV1.at(context)) {
      final double bottom =
...
/// A [FormField] that contains a [DropdownButtonV1].
///
/// This is a convenience widget that wraps a [DropdownButtonV1] widget in a
/// [FormField].
///
...
/// See also:
///
///  * [DropdownButtonV1], which is the underlying text field without the [Form]
///    integration.
class DropdownButtonFormFieldV1<T> extends FormField<T> {
  /// Creates a [DropdownButtonV1] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  /// For a description of the `onSaved`, `validator`, or `autovalidateMode`
  /// parameters, see [FormField]. For the rest (other than [decoration]), see
  /// [DropdownButtonV1].
  DropdownButtonFormFieldV1({
    super.key,
    required List<DropdownMenuItemV1<T>>? items,
        ...
        super(
          ...
          builder: (FormFieldState<T> field) {
            ...
            final bool showSelectedItem = items != null &&
                items
                    .where(
                      (DropdownMenuItemV1<T> item) => item.value == state.value,
                    )
                    .isNotEmpty;
            ...
            return Focus(
              ...
              child: Builder(
                builder: (BuildContext context) {
                  ...
                  return DropdownButtonHideUnderlineV1(
                    child: DropdownButtonV1<T>._formField(
                      items: items,
...
class _DropdownButtonFormFieldState<T> extends FormFieldState<T> {
  DropdownButtonFormFieldV1<T> get _dropdownButtonFormField =>
      widget as DropdownButtonFormFieldV1<T>;
  ...
  @override
  void didUpdateWidget(DropdownButtonFormFieldV1<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
